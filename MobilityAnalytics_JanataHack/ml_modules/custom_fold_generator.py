# %load ml_auto/custom_fold_generator.py
from sklearn.model_selection import StratifiedKFold, KFold, GroupKFold, GroupShuffleSplit, TimeSeriesSplit, train_test_split
...
class CustomFolds(object):
    def __init__(self, validation_scheme=FoldScheme.StratifiedKFold, num_folds=5, random_state=100, num_repeats=1, shuffle=True, test_size=0.2):
    ...
    def split(self, X, y=None, group=None, **kwargs):
        ...
        if isinstance(self.validation_scheme, FoldScheme):
            ...
            elif self.validation_scheme == FoldScheme.GroupShuffleSplit:
                folds = GroupShuffleSplit(n_splits=self.num_folds, random_state=self.random_state)
                return [(train_index, test_index) for (train_index, test_index) in folds.split(X, y, groups=group)]
